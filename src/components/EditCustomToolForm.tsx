...
import { ITool, Language, toolList } from "@/constant/tool_list"
import { classify, LANGUAGE_LIBRARY } from "@/constant/language"
import { useAppDispatch, useAppSelector } from "@/app/store/hooks"
import { getAllClassifyData } from "@/app/api/classify/indexedDB"
import { selectGlobal, setGlobalState } from "@/app/store/globalSlice"
import { updateCustomToolData, getAllCustomToolData } from "@/app/api/customTool/indexedDB"
...
export function EditCustomToolForm(params: { dataSource: ITool & { prompt?: string }; disabled: boolean }) {
  const { dataSource, disabled } = params;
  ...
  const form = useForm<z.infer<typeof FormSchema>>({
    ...
    defaultValues: {
      name: dataSource?.name?.chinese || '',
      describe: dataSource?.describe?.chinese || '',
      classify_key: dataSource.classify.english || '',
      prompt: dataSource?.prompt || '',
      url: dataSource?.url || '/default.png'
    },
  })

  const onSubmit = async (data: z.infer<typeof FormSchema>) => {
    setLoad(true)
    if (onJudgeRepeatTool(data.name) && data.name !== dataSource.name.chinese) {
      toast({
        duration: 2000,
    ...
    if (classifyData) {
      await updateCustomToolData(+dataSource.id, {
      ...
      if (data.name === dataSource.name.chinese) {
        setLoad(false); setOpen(false);
      } else {
        router.replace(`/${data.name.replace(/\s+/g, '-')}`)
      }
    } else {
      toast({
        duration: 2000,
  ...
  const onRefreshData = async () => {
    ...
    dispatch(setGlobalState({ customTool: customToolData, classify: classifyData }))
  }
